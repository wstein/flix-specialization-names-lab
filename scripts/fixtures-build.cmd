@echo off
rem Runs fixtures-build.scala against a Flix compiler jar. See scripts/fixtures-build
rem (the POSIX wrapper) for why this exists: a `using jar` directive is a static
rem literal, so fixtures-build.scala alone has no classpath.
rem
rem This never goes through flixw.cmd, on purpose, same reason as the other lab scripts:
rem flixw.cmd holds only one pinned jar at a time, and building the fixtures against an
...
rem
rem Like scripts/fixtures-check.cmd, this one always changes to the project root: it has
rem no user-supplied path argument to accidentally resolve against the wrong directory --
rem it only ever builds this project's own fixtures\positive\ into build\fixtures\.
rem
rem Usage:
rem   scripts\fixtures-build.cmd
rem   scripts\fixtures-build.cmd --flix-jar path\to\flix.jar
rem   scripts\fixtures-build.cmd --out build\fixtures
...
:usage
echo Usage: scripts\fixtures-build.cmd [--flix-jar path\to\flix.jar] [--compiler-version string] [--out dir]
echo(
echo Compiles every file under fixtures\positive\ for real and writes each one's generated
echo classes to its own subdirectory of build\fixtures\ (or --out^), so scripts\class-id-report
echo can census a single fixture in isolation. fixtures\negative\ is skipped: it is built to
echo fail, by design, so there is nothing to write.
echo(
echo Options:
...
echo   --out ^<dir^>              Where to write classes. Default: build\fixtures,
echo                             wiped before every run.
...
scala-cli run "%~dp0fixtures-build.scala" --jvm 21 --jar "%JAR%" -- --compiler-version "%COMPILER_VERSION%" %ARGS%
